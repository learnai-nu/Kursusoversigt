---
title: "ChatGPT-kurser i Danmark: oversigt og hvordan du vælger"
description: "Hvilke ChatGPT- og generativ AI-kurser findes i Danmark, hvordan adskiller de sig fra OpenAI Academy — og hvad skal du vælge til dig eller dit team?"
pubDate: 2026-09-08
author: "Mette Ravn"
tags: ["chatgpt", "generativ-ai", "kursusvalg", "danmark"]
sources:
  - title: "OpenAI Academy"
    url: https://academy.openai.com/pages/courses
  - title: "Digital Forlomme — ChatGPT-kursus"
    url: https://digitalforlomme.dk
  - title: "Promptly"
    url: https://promptly.dk
---

ChatGPT er ofte indgangen til generativ AI på danske arbejdspladser. Det betyder også, at “ChatGPT-kursus” dækker alt fra en dags workshop til længere forløb i prompt engineering, sikker brug og team-workflows. Her er en praktisk oversigt, så du kan matche niveau og format — uden hype.

## Hvad et godt ChatGPT-kursus bør dække

Et solidt forløb går ud over “skriv en prompt”. Det bør typisk rumme:

- klare instruktioner og kontekst (hvad skal output bruges til?)
- review og kvalitetskontrol (hvornår stoler I *ikke* på svaret?)
- data og fortrolighed (hvad må I indtaste?)
- overførsel til jeres egne opgaver (mail, møder, dokumenter, support)

Hvis I primært skal beslutte strategi eller governance, er et [leder-orienteret forløb](/ai-kurser/itu-ai-for-ledere) ofte bedre end et rent værktøjskursus. Skal teamet *bruge* værktøjet i hverdagen, så kig efter praktiske workshops.

## Danske udbydere med ChatGPT i fokus

**Korte, praktiske dage**

- [ChatGPT-kursus (1 dag) hos Digital Forlomme](/ai-kurser/digital-forlomme-chatgpt) — konkret værktøjsdag.
- [ChatGPT og AI på kontoret (Dania)](/ai-kurser/dania-chatgpt-for-kontoret) — kontor- og administrationsvinkel.
- [AI i arbejdsdagen – ChatGPT og Copilot (Montus)](/ai-kurser/montus-ai-i-arbejdsdagen) — blander ChatGPT med Copilot.
- [AI kursus hos IBC](/ai-kurser/ibc-ai-kursus) — ChatGPT og andre AI-værktøjer for fagprofessionelle.

**Fra grundkursus til avanceret**

- [AI Grundkursus hos Promptly](/ai-kurser/promptly-ai-grundkursus) — sikker og praktisk start.
- [Avanceret ChatGPT hos Promptly](/ai-kurser/promptly-avanceret-chatgpt) — næste niveau for erfarne brugere.
- [Prompt engineering for praktikere (KEA)](/ai-kurser/kea-prompt-engineering) — mere systematisk prompting.
- [Generativ AI i praksis (Teknologisk Institut)](/ai-kurser/ti-generativ-ai-i-praksis) — bredere generativ AI end kun ChatGPT.

Se også det fulde katalog under [AI-kurser](/ai-kurser) og filtrer på praktisk niveau eller online/fremmøde.

## OpenAI Academy vs. dansk kursus

[OpenAI Academy — AI Foundations](/ai-kurser/openai-academy-ai-foundations) (og de øvrige Academy-forløb) er **globale, ofte self-paced** forløb fra OpenAI. De er stærke til fundament og certificeringslignende completion — men de er ikke et dansk hold med lokal underviser, DK-cases eller virksomhedstilpasning.

Vælg Academy, når I vil have officielt produktfokus og kan arbejde asynkront. Vælg en dansk udbyder, når I vil have dialog, holddynamik, dansk kontekst eller et skræddersyet teamforløb. Se hele gratis-sporet i [Gratis AI-kurser: Claude, OpenAI og Google](/artikler/gratis-ai-kurser).

## Sådan vælger du i praksis

1. **Outcome først:** Skal I spare tid i mail, forbedre kundesvar, eller bygge interne guides?
2. **Niveau:** Begynder-workshop vs. avanceret prompting — bland ikke de to på samme hold uden plan.
3. **Format:** Én dag kan kickstarte; længere forløb (eller [LearnAI team-workshop](/ai-kurser/learnai-team-workshop)) skaber vaner.
4. **Pris og dato:** På Kursusoversigten står ukendte priser som **Ukendt** med vilje. Tjek altid udbyderens egen side via kilde-URL’en.

Læs også [Sådan vælger du AI-kursus i Danmark](/artikler/vaelg-ai-kursus-danmark) og [Generativ AI på arbejdspladsen](/artikler/generativ-ai-pa-arbejdspladsen).

Når du er klar til sparring om, hvilket ChatGPT- eller teamforløb der matcher jeres behov, kan [LearnAI](https://learnai.nu/?utm_source=kursusoversigt&utm_medium=referral&utm_campaign=chatgpt-article) hjælpe med at oversætte kataloget til en konkret læringsplan.

Hvis målet er bedre beslutninger som leder, er et kursus i [AI for ledere](/ai-kurser/itu-ai-for-ledere) eller [AI for danske ledere](/ai-kurser/learnai-lederkursus) mere relevant end deep learning. Se hele lederoversigten: [AI-kurser for ledere 2026](/artikler/ai-kurser-for-ledere-2026).
